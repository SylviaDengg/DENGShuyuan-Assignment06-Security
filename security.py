...
class RateLimiter:
    ...
    def __init__(self, max_requests: int = 10, window_seconds: int = 60, time_func: Callable[[], float] | None = None) -> None:
...
class EthicalGuard:
    ...
    def __init__(self, log_path: str = "flagged_content.jsonl") -> None:
        ...
        self.patterns = {
            ...
            "prompt_injection": [r"ignore previous instructions", r"reveal (the )?system prompt", r"\bsystem prompt\b"],
        }
...
def secure_process_request(
    ...
    if not output_safe:
        return {"ok": False, "stage": "output_guard", "message": "Model output was blocked because it matched an unsafe pattern.", "sanitized_input": sanitized}
    return {"ok": True, "stage": "completed", "message": "Request processed successfully.", "sanitized_input": sanitized, "output": output}
